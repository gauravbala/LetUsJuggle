...
class Main:
    ...
    def drawBoard(self):
        # Guess column
        self.canvas.create_rectangle(self.margin, self.margin, self.windowWidth-self.topMargin, 
            self.windowHeight-self.resultMargin, fill="sandy brown", outline="saddle brown")
        # Result column
        self.canvas.create_rectangle(self.margin, self.windowHeight-self.resultMargin,
            self.windowWidth-self.topMargin, self.windowHeight-self.margin, fill="NavajoWhite2", 
    ...
    def getGuessCoords(self, row, circle):
        ...
        x0 = self.margin + self.lightMargin + (row*self.guessWidths)
        x1 = x0 + self.guessWidths - 2*self.lightMargin
        y0 = self.margin + self.lightMargin + (circle*height)
    ...
    def getLightCoords(self, row, circle):
        ...
        x0 = self.margin + self.lightMargin + (row*self.guessWidths) + self.guessWidths/2 - diameter
        x1 = x0 + diameter
        y0 = self.windowHeight - self.resultMargin + self.lightMargin + (circle*(height/4))
        y1 = y0 + diameter
        return (x0, y0, x1, y1)
    ...
    def drawFinalResult(self):
        if (self.game.win):
            self.canvas.create_text(self.windowWidth-self.topMargin/2, self.windowHeight/2, 
                text="YOU WIN!", font="Courier 10")
        else:
            self.canvas.create_text(self.windowWidth-self.topMargin/2, self.windowHeight/3, 
                text="YOU LOSE\nRESULT WAS:", font="Courier 10")
            for circle in range(4):
                diameter = self.topMargin - 4*self.margin
                x0 = self.windowWidth - self.topMargin/2 - diameter/2
